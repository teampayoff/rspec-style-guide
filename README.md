...
This RSpec style guide outlines our recommended best practices so that
our
...
The guide is separated into sections based on the different pieces of an
entire 
spec file. There was an attempt to omit all obvious information, if
anything is
...
simply
lacking thorough examples, but some things in the RSpec world change
week by 
week or month by month. With that said, as the standard changes this
guide is 
...
descriptions. It does makes the code more difficult to read and lowers
the
...
Leave one line return after `let`, `subject`, and `before`/`after`
blocks.
...
Only group `let`, `subject` blocks and separate them from
`before`/`after`
...
There is no need to specify `(:each)` for `before`/`after` blocks, as it
is
the default functionality. There are almost zero cases to use
`before(:all)`
...
blocks.
The descriptions represent actual functionality - not what might be
happening.
...
Use only one expectation per example. There are very few scenarios where
two
or more expectations in a single `it` block should be used. So, general
rule
...
`context` blocks should pretty much always have an opposite negative
case. It
should actually be a strong code smell if there is a single context
(without a
matching negative case) that it needs refactoring, or may have no
purpose.
...
`context` block descriptions should always start with 'when', and be in
the
...
`it` block descriptions should never end with a conditional. This is a
code
...
feature to one of the items in the iteration, he must then break it out
into a
separate test - he is forced to edit code that has nothing to do with
his pull
request.
...
Use [Factory Girl](https://github.com/thoughtbot/factory_girl) to create
test
objects in integration tests. You should very rarely have to use
`ModelName.create` within an integration spec. Do **not** use fixtures
as they
...
Use mocks and stubs with caution. While they help to improve the
performance
of the test suite, you can mock/stub yourself into a false-positive
state very
easily. When resorting to mocking and stubbing, only mock against a
small,
stable, obvious (or documented) API, so stubs are likely to represent
reality
...
*NOTE*: if you stub a method that could give a false-positive test
result, you 
...
Always use [Timecop](https://github.com/travisjeffery/timecop) instead
of
...
it 'offsets the time 2 days into the future' do
  ...
  expect(subject.get_offset_time).to be_the_same_time_as (current_time +
2.days)
end
```
...
it 'offsets the time 2 days into the future' do
  Timecop.freeze(Time.now) do
    expect(subject.get_offset_time).to be_the_same_time_as
2.days.from_now
  end
end
```

*NOTE*: `#be_the_same_time_as` is a RSpec matcher we added to the
platform, it
...
the spec
examples. `let` blocks get lazily evaluated. It also removes the
instance
variables from the test suite (which don't look as nice as local
variables).

These should primarily be used when you have duplication among a number
of
`it` blocks within a `context` but not all of them. Be careful with
overuse of
...
Use RSpec's 'magical matcher' methods when possible. For instance, a
class
...
into a
shared environment too early, as this can lead to brittle tests that
rely too
much on one other.

It general it is best to start with doing everything directly in your
`it`
blocks even if it is duplication and then refactor you tests after you
have
them working to be a little more DRY. However, keep in mind that
duplication
in test suites is NOT fround upon, in fact it is preferred if it
provides
...
* Prefer the capybara negative selectors over should_not with the
  positive.
...
* Mock the models and stub their methods. Testing the controller should
  not 
...
* Test only the behaviour the controller should be responsible about:
  ...
  * Result from the action - template render, redirect, etc.
        ...
        # We are interested only in the actions the controller should
perform
        # So we are mocking the model creation and stubbing its methods
        # And we concentrate only on the things the controller should do

        describe ArticlesController do
          # The model will be used in the specs for all methods of the
controller
          ...
          describe 'POST create' do
            ...
            it 'creates a new article with the given attributes' do
              expect(Article).to receive(:new).with(title: 'The New
Article Title').and_return(article)
...
* Use context when the controller action has different behaviour
  depending on 
  the received params.

    ```ruby
    # A classic example for use of contexts in a controller spec is
creation or update when the object saves successfully or not.
    ...
    describe ArticlesController do
      ...
      describe 'POST create' do
        ...
        it 'creates a new article with the given attributes' do
          expect(Article).to receive(:new).with(title: 'The New Article
Title').and_return(article)
        ...
        context 'when the article saves successfully' do
          ...
          it 'sets a flash[:notice] message' do
            post :create
            expect(flash[:notice]).to eq('The article was saved
successfully.')
          end
...
* Use `FactoryGirl.create` to make real objects, or just use a new
  (unsaved) 
...
* When testing validations, use `have(x).errors_on` to specify the
  attibute
  which should be validated. Using `be_valid` does not guarantee that
the 
...
* When testing uniqueness of a model attribute, name the other object 
  `another_object`.
    ...
    describe Article do
      describe '#title' do
        it 'is unique' do
          another_article = FactoryGirl.create(:article, title:
article.title)
          expect(article).to have(1).error_on(:title)
        end
      end
...
* The model in the mailer spec should be mocked. The mailer should not
  depend on
  the model creation.
* The mailer spec should verify that:
  ...
  * the e-mail contains the required information
    ...
    describe SubscriberMailer do
      let(:subscriber) { double(Subscription, email: 'johndoe@test.com',
name: 'John Doe') }

      describe 'successful registration email' do
        subject {
SubscriptionMailer.successful_registration_email(subscriber) }
...
[howaboutwe's rspec style
guide](https://github.com/howaboutwe/rspec-style-guide)

[bbatsov's rspec style
